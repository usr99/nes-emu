...
mod test {
	...
	fn bcc() {
		let mut cpu = MOS6502::new();
		cpu.load_and_run(&[0x90, 0x02, 0x00, 0x00, 0xa9, 0x42, 0x00]);
		assert_eq!(cpu.reg.acc, 0x42);
	}
	...
	fn bcs() {
		let mut cpu = MOS6502::new();
		cpu.load(&[0xb0, 0x02, 0x00, 0x00, 0xa9, 0x42, 0x00]);
		cpu.reset();
		cpu.reg.status |= StatusFlags::CARRY;
		cpu.run();
		assert_eq!(cpu.reg.acc, 0x42);
	...
	fn beq() {
		let mut cpu = MOS6502::new();
		cpu.load(&[0xf0, 0x02, 0x00, 0x00, 0xa9, 0x42, 0x00]);
		cpu.reset();
	...
	fn bmi() {
		let mut cpu = MOS6502::new();
		cpu.load(&[0x30, 0x02, 0x00, 0x00, 0xa9, 0x42, 0x00]);
		cpu.reset();
	...
	fn bne() {
		let mut cpu = MOS6502::new();
		cpu.load_and_run(&[0xd0, 0x02, 0x00, 0x00, 0xa9, 0x42, 0x00]);
		assert_eq!(cpu.reg.acc, 0x42);
	}
	...
	fn bpl() {
		let mut cpu = MOS6502::new();
		cpu.load_and_run(&[0x10, 0x02, 0x00, 0x00, 0xa9, 0x42, 0x00]);
		assert_eq!(cpu.reg.acc, 0x42);
	}
	...
	fn bvc() {
		let mut cpu = MOS6502::new();
		cpu.load_and_run(&[0x50, 0x02, 0x00, 0x00, 0xa9, 0x42, 0x00]);
		assert_eq!(cpu.reg.acc, 0x42);
	}
	...
	fn bvs() {
		let mut cpu = MOS6502::new();
		cpu.load(&[0x70, 0x02, 0x00, 0x00, 0xa9, 0x42, 0x00]);
		cpu.reset();
